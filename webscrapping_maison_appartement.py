...
def get_logement_2021(url):
    ...
    colonnes_2021 = [col for col in df.columns if "2021" in col or "Catégorie" in col]
    df_2021 = df[colonnes_2021].copy()

    return df_2021.set_index("Catégorie ou type de logement - Ensemble").T[["Maisons", "Appartements"]]
...
if "Catégorie ou type de logement - Ensemble" in df_final.columns:
    df_final.drop(columns=["Catégorie ou type de logement - Ensemble"], inplace=True)

df_final.to_csv("data_maisons_appartements.csv", index=False, encoding="utf-8-sig")

df = pd.read_csv("data_maisons_appartements.csv")
...
df.to_csv("data_maisons_appartements.csv", index=False, encoding="utf-8-sig")
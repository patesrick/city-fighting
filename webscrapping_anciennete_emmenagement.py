...
def get_sexe(url):
    ...
    table = soup.find("table", {"id": "produit-tableau-LOG_G2"})
...
df_final.to_csv("data_anciennete_emmenagement.csv", index=False, encoding="utf-8-sig")

df = pd.read_csv("data_anciennete_emmenagement.csv")
...
df.to_csv("data_anciennete_emmenagement.csv", index=False, encoding="utf-8-sig")
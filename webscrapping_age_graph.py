...
def get_sexe(url):
    ...
    table = soup.find("table", {"id": "produit-tableau-POP_T1"})
    ...
    colonnes_a_supprimer = [col for col in df.columns if " " in col]
...
df_final.to_csv("data_age_graph.csv", index=False, encoding="utf-8-sig")

df = pd.read_csv("data_age_graph.csv")
...
df.to_csv("data_age_graph.csv", index=False, encoding="utf-8-sig")
...
def get_sexe(url):
    ...
    table = soup.find("table", {"id": "produit-tableau-POP_T5"})
...
df_final.to_csv("data_cat_socio.csv", index=False, encoding="utf-8-sig")

df = pd.read_csv("data_cat_socio.csv")
...
df.to_csv("data_cat_socio.csv", index=False, encoding="utf-8-sig")
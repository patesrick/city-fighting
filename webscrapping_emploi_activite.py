...
def get_sexe(url):
    ...
    table = soup.find("table", {"id": "produit-tableau-EMP_T5"})
...
df_final.to_csv("data_emploi_activite.csv", index=False, encoding="utf-8-sig")

df = pd.read_csv("data_emploi_activite.csv")
...
df.to_csv("data_emploi_activite.csv", index=False, encoding="utf-8-sig")
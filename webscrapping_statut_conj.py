...
def get_sexe(url):
    ...
    table = soup.find("table", {"id": "produit-tableau-FAM_G4"})
...
df_final.to_csv("data_statut_conj.csv", index=False, encoding="utf-8-sig")

df = pd.read_csv("data_statut_conj.csv")
...
df.to_csv("data_statut_conj.csv", index=False, encoding="utf-8-sig")
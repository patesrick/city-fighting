...
def get_sexe(url):
    ...
    table = soup.find("table", {"id": "produit-tableau-LOG_T4"})
...
df_final.to_csv("data_piece_moy.csv", index=False, encoding="utf-8-sig")

df = pd.read_csv("data_piece_moy.csv")
...
df.to_csv("data_piece_moy.csv", index=False, encoding="utf-8-sig")